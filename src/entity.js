class Entity {
    ...
    if (this.image) {
      ...
      if (debug) {
        ...
        c.strokeRect(
          pos_x,
          pos_y,
          this.image.resolution[0],
          this.image.resolution[1],
        );
      }
    }
    if (this.animation) {
      ...
      if (debug) {
        ...
        c.strokeRect(
          pos_x,
          pos_y,
          this.animation.desc.resolution[0],
          this.animation.desc.resolution[1],
        );
      }
    }
  ...
  update(game) {
    ...
    let inBounds =
      leftNext > -1 && rightNext < 20 && topNext > -1 && botNext < 15;

    if (leftNow > leftNext && inBounds) {
      for (let y = topNow; y <= botNow; y++) {
        if (map.tiles[leftNext][y]?.wall) {
          this.velocity[0] = 0;
